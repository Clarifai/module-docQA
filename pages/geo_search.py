...
from utils.geo_search_utils import (display_location_info, get_location_data,
                                    ...
                                    llm_output_to_json,
                                    process_post_searches_response,
...
if task_query:
    ...
    prompt = PromptTemplate(
        template=NER_LOC_RADIUS_PROMPT, input_variables=["page_content"]
    )
    ...
    # If DataFrame is not empty, proceed with displaying and summarizing searches
        ...
        input_df["lat"] = input_df["lat"] + np.where(
            random_noise_1 > 0, random_noise_1, random_noise_2
        )
        input_df["lon"] = input_df["lon"] + np.where(
            random_noise_1 > 0, random_noise_2, random_noise_1
        )
