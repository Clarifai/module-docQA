...
# clarifai_qa = st.tabs(["Clarifai Q&A"])
...
if user_input:
    ...
    if docs != []:
        ...
        def process_input(input):
            chat_output = get_openai_output(llm_chain, input["page_content"])
        ...
        if doc_selection and doc_selection != "-":
            ...
            @st.cache_resource
            def get_full_text(doc_selection):
                auth = ClarifaiAuthHelper.from_streamlit(st)
                stub = create_stub(auth)
                userDataObject = auth.get_user_app_id_proto()
            ...
            def get_summarization_output(doc_selection):
                full_text = get_full_text(doc_selection)
...
#     st.markdown("CHAT")
# text_splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=0)
# st.write("dsadas", st.session_state.full_text)
# if st.session_state.full_text != "":
#     documents = text_splitter.create_documents([st.session_state.full_text])
