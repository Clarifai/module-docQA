...
class Clarifai(VectorStore):
  ...
  def __init__(
    ...
    try:
      ...
      raise ValueError("Could not import clarifai python package. "
  ...
  def similarity_search(
    ...
    post_annotations_searches_response = self._stub.PostAnnotationsSearches(
        service_pb2.PostAnnotationsSearchesRequest(
            ...
            searches=[
                resources_pb2.Search(query=resources_pb2.Query(ranks=[
    ...
    if post_annotations_searches_response.status.code != status_code_pb2.SUCCESS:
      ...
      raise Exception("Post searches failed, status: " +
    ...
    for hit in hits:
      ...
      if "source" in metadata:
        ...
        source = hit.input.id
        page_number = "NA"
      print("\tScore %.2f for annotation: %s off input: %s, source: %s, text: %s" % (
          hit.score,
          hit.annotation.id,
          hit.input.id,
          source,
